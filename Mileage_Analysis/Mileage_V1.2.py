...
def data_init(filename: str) -> tuple:
    ...
    avaible_data_index_flag = (1 if not avaible_data_index else 0)  # V1.2新增，可用数据判断标志位，无可用数据为1
    ...
    return data_result, avaible_data_index_flag
...
def main():
    ...
    for path in paths:
        ...
        for file in files:
            ...
            data, avaible_data_index_flag = data_init(file)  # 数据清洗
            if avaible_data_index_flag:
                continue
            else:
                mil_aver_soc, data_error_need, flag_len_a = milesge_aver(data)
                if flag_len_a:
                    print('\033[0;30;41m{} 用时 {} 秒 无可用数据,未计入结果！请检查车辆数据!\033[0m'.format
                          (file[0:17], (datetime.datetime.now() - start_).total_seconds()))  # 接上一行
                    continue
            ...
            if mil_aver_quantity > 5:
                ...
                print('{} 能耗 -> {:<4}, 用时 {:<8} 秒'.format(file[0:17], mil_aver_quantity,
